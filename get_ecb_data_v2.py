...
def get_mention_doc(root, doc_name, validated_sentences):
    ...
    for mention in root.find('Markables'):
        ...
        if 'RELATED_TO' not in mention.attrib:
            ...
            # bert_start_index = int(np.searchsorted(alignment_ecb_bert_ids, tokens_ids[0], side='left'))
            # bert_end_index = int(np.searchsorted(alignment_ecb_bert_ids, tokens_ids[-1], side='right'))

            # first_token_in_sentence = int(np.searchsorted(sentence_alignment, int(sentence), side='left'))
            # bert_start_sentence = bert_start_index - first_token_in_sentence
            # bert_end_sentence = bert_end_index - first_token_in_sentence
            #
            # if bert_start_index == bert_end_index:
            #     raise ValueError(file_name, m_id)
